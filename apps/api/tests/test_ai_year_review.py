...
def test_year_review_generate_and_fetch(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert body["growth"]["goals_completed"] == 2
        ...
        try:
            ...
            assert "成为销售经理" in titles
            assert any("布达拉宫" in record.get("content", "") for record in content.input_json["records"])
        finally:
            db.close()
...
def test_year_review_user_isolation(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        other = client.post("/api/v1/ai/year-summary", headers=_headers(user_b), json={"year": date.today().year})
