  ...
  return (
    <div className="space-y-4">
      <Card className="p-4">
        ...
        {(recommendations.length > 0 || searchResults.length > 0) && <div className="mt-4 grid gap-3 md:grid-cols-2">{[...recommendations.map((item) => ({ ...item, recommendation: true })), ...searchResults].map((item: any, index) => <div key={`${item.title}-${item.url || index}`} className="rounded-xl border border-border p-3"><div className="flex items-start justify-between gap-2"><p className="text-sm font-semibold">{item.title}</p>{item.isComplete && <Badge variant="success">完整书籍</Badge>}</div><p className="mt-1 text-xs text-muted">{item.author || item.sourceName || item.provider}</p><p className="mt-2 line-clamp-3 text-xs text-text-secondary">{item.description || item.reason || item.snippet || "可加入书单后设置自己的阅读计划。"}</p><div className="mt-3 flex gap-2"><Button size="sm" onClick={() => void addBook(item)} disabled={createBook.isPending || searching}><Plus className="h-3.5 w-3.5" />加入想看</Button>{item.url && <a href={item.url} target="_blank" rel="noreferrer"><Button size="sm" variant="outline">查看来源<ExternalLink className="h-3.5 w-3.5" /></Button></a>}</div></div>)}</div>}
      </Card>
